$(document).ready(function(){
  ...
  $(document).on('click', '.agregar-producto', (e)=>{
    ...
    if(producto_select2==null){
      ...
      if(codigo_lote==''){
        ...
        if(cantidad==''){
          $('#error').text('¡Elija una cantidad!');
          $('#noadd-prod').hide('slow');
          $('#noadd-prod').show(1000);
          $('#noadd-prod').hide(3000);
        }else{
          if(vencimiento==''){
            $('#error').text('¡Introduzca una fecha de vencimiento!');
            $('#noadd-prod').hide('slow');
            $('#noadd-prod').show(1000);
            $('#noadd-prod').hide(3000);
          }else{
            if(precio_compra==''){
              $('#error').text('¡Introduzca un precio de compra!');
              $('#noadd-prod').hide('slow');
              $('#noadd-prod').show(1000);
              $('#noadd-prod').hide(3000);
            }else{
  ...
  $(document).on('click', '.crear-compra', (e)=>{
    ...
    if(codigo==null){
      $('#error-compra').text('Introduzca un código!');
      $('#noadd-compra').hide('slow');
      $('#noadd-compra').show(1000);
      $('#noadd-compra').hide(3000);
    }else{
      if(fecha_compra==''){
        $('#error-compra').text('Introduzca una fecha de compra!');
        $('#noadd-compra').hide('slow');
        $('#noadd-compra').show(1000);
        $('#noadd-compra').hide(3000);
      }else{
        if(fecha_entrega==''){
          $('#error-compra').text('Introduzca una fecha de entrega!');
          $('#noadd-compra').hide('slow');
          $('#noadd-compra').show(1000);
          $('#noadd-compra').hide(3000);
        }else{
          if(total==''){
            $('#error-compra').text('Introduzca un total!');
            $('#noadd-compra').hide('slow');
            $('#noadd-compra').show(1000);
            $('#noadd-compra').hide(3000);
          }else{
